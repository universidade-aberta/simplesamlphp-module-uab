...
class SP extends \SimpleSAML\Module\saml\Auth\Source\SP {
    ...
    /**
     * Add UAb custom extensions for SAML Authentication
     * - Request extra attributes
     * - FAAALevel
     *
     * @param \SAML2\AuthnRequest $ar An authentication request object.
     * @return \SAML2\AuthnRequest An authentication request object.
     * @autor Cláudio Esperança <cesperanc@gmail>
     * @UAb
     */
    protected function UAb_AddCustomExtensions(AuthnRequest $ar):AuthnRequest{
        $extensions = [];

        // RequestedAttributes
        $requestAttributesMeta = $this->metadata->getOptionalArray('RequestAttributes', null);
        if (!empty($requestAttributesMeta['attributes']) && is_array($requestAttributesMeta['attributes'])):

            $dom = \SAML2\DOMDocumentFactory::create();

            $attributesNS = $requestAttributesMeta['namespace']??null;
            if ($attributesNS !== null):
                $requestedAttributes = $dom->createElementNS(
                    $attributesNS,
                    'fa:RequestedAttributes'
                );
            else:
                $requestedAttributes = $dom->createElement(
                    'RequestedAttributes'
                );
            endif;
            ...
            foreach($requestAttributesMeta['attributes'] as $requestAttributeMetaName=>$requestAttributeMeta):
                $requestedAttribute = $dom->createElement(
                    'fa:RequestedAttribute'
                );

                foreach($attributeKeysMap as $requestAttributeKey=>$requestAttributeName):
                    if(isset($requestAttributeMeta[$requestAttributeKey])):
                        $domAttribute = $dom->createAttribute($requestAttributeName);
                        $value = $requestAttributeMeta[$requestAttributeKey];
                        if(is_bool($value)):
                            $value = $value?'true':'false';
                        endif;
                        $domAttribute->value = (string) $value;
                        $requestedAttribute->appendChild($domAttribute);
                    endif;
                endforeach;
            ...
            $extensions[] = new \SAML2\XML\Chunk($requestedAttributes);
        endif;

        //FAAALevel
        $FAAALevelMeta = $this->metadata->getOptionalArray('FAAALevel', null);
        if ($FAAALevelMeta!==null && !empty($FAAALevelMeta['value'])):
            $dom = \SAML2\DOMDocumentFactory::create();

            $FAAALevelMetaNS = $FAAALevelMeta['namespace']??null;
            if ($FAAALevelMetaNS !== null):
                $FAAALevel = $dom->createElementNS(
                    $attributesNS,
                    'fa:FAAALevel', 
                    (string)$FAAALevelMeta['value']
                );
            else:
                $FAAALevel = $dom->createElement(
                    'FAAALevel', 
                    (string)$FAAALevelMeta['value']
                );
            endif;

            $extensions[] = new \SAML2\XML\Chunk($FAAALevel);
        endif;
            ...
            switch ($service) {
                case Constants::BINDING_HTTP_POST:
                    $acs = [
                        ...
                        //@UAb: update to use our custom route
                        'Location' => Module::getModuleURL('uab/sp/saml2-acs.php/' . $this->getAuthId()),
                    ];
                    break;
    ...
    private function getSLOEndpoints(): array
    {
        ...
        //@UAb: update to use our custom route
        $defaultLocation = Module::getModuleURL('uab/sp/saml2-logout.php/' . $this->getAuthId());
    ...
    private function startSSO2(Configuration $idpMetadata, array $state): void
    {
        ...
        //@UAb: Use our custom Message
        $ar = Message::buildAuthnRequest($this->metadata, $idpMetadata);

        //@UAb: update to use our custom route
        $ar->setAssertionConsumerServiceURL(Module::getModuleURL('uab/sp/saml2-acs.php/' . $this->authId));
        ...
        //@UAb{
        $this->UAb_AddCustomExtensions($ar);
        //@UAb}
    ...
    public function startSSO(string $idp, array $state): void
    {
        ...
        switch ($type) {
            case 'saml20-idp-remote':
                self::startSSO2($idpMetadata, $state);
